...
export function PinnedEntryCard({ entry }: Props) {
  ...
  const firstImage = entry.images?.[0] ?? entry.imageDataUrl ?? storedImageUrls[0]
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        {firstImage ? (
          <img src={firstImage} alt={entry.name}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1">
            ...
            <Star className="w-2.5 h-2.5 text-amber-500 fill-amber-500 flex-shrink-0 star-glow" />
          </div>
